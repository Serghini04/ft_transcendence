...
export function generateJwtAccessToken({id, name, email}: {id: number; name:string; email: string}){
    ...
    const token = jwt.sign({ id: id, name: name, email: email }, process.env.JWT_SECRET, { expiresIn: "15s" })
    return token;
}
...
export function generateJwtRefreshToken({id, name, email}: {id: number; name:string; email: string}){
  ...
  const token = jwt.sign({ id: id, name: name, email: email }, process.env.JWT_REFRESH, { expiresIn: "20s" })
  return token;
}
