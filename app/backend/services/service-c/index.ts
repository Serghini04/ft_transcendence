...
const consumer = new KafkaConsumerService({
  ...
  // Custom message handler for service-c specific logic
  messageHandler: async (messageData: MessageData): Promise<void> => {
    console.log('Processing in Service-C:', messageData.value);
  }
});
...
const start = async (): Promise<void> => {
  try {
    ...
    console.log(`Service-C API listening on port ${PORT}`);
  } catch (err) {
...
const closeGracefully = async (signal: string): Promise<void> => {
  console.log(`${signal} signal received: closing Service-C`);
